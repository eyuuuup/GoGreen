...
public class Food {
    ...
    public static void addAction(boolean veggie, boolean locally, boolean bio, String description)
        ...
        if (veggie) {
            if (!Communication.addAction("Meat", 50, carbonReduced, 0, description)) {
                throw new IllegalArgumentException(exception);
            }
        }

        if (locally) {
            if (!Communication.addAction("Local", 50, carbonReduced, 0, description)) {
                throw new IllegalArgumentException(exception);
            }
        }

        if (bio) {
            if (!Communication.addAction("Biological", 50, carbonReduced, 0, description)) {
                throw new IllegalArgumentException(exception);
            }
        }
    }
